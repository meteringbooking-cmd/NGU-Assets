...
import { el, toast, openModal, closeModal, assetRecordQrUrl } from "./common.js";
...
function buildLabelImage({ qrCode, assetId }, settings) {
  return new Promise((resolve) => {
    ...
    // Wide labels get the QR and Asset ID side by side (the original
    // layout). Square or tall labels — like the 0.5in x 0.5in stock this
    // printer actually has loaded — stack the QR on top with the Asset ID
    // centered underneath, since there's no room to put them side by side.
    const stacked = canvas.width < canvas.height * 1.3;

    let qrSize, qrX, qrY;
    if (stacked) {
      qrSize = Math.round(Math.min(canvas.width - 8, canvas.height * 0.7));
      qrX = Math.round((canvas.width - qrSize) / 2);
      qrY = 4;
    } else {
      qrSize = canvas.height - 8;
      qrX = 4;
      qrY = 4;
    }
    ...
    // matches shared/common.js's buildQrLabel (the browser-print version).
    // Error-correction level L (not the library default M) keeps the
    // printed pattern as sparse as possible — critical on a label this
    // small, where a denser pattern can be too fine for a phone camera to
    // resolve at all.
    // eslint-disable-next-line no-new
    new window.QRCode(qrHost, {
      text: assetId ? assetRecordQrUrl(assetId) : "",
      width: qrSize,
      height: qrSize,
    ...
    setTimeout(() => {
      ...
      if (stacked) {
        const textAreaTop = qrY + qrSize + 2;
        const textAreaHeight = Math.max(8, canvas.height - textAreaTop - 2);
        drawFittedLine(ctx, assetId || "", canvas.width / 2, textAreaTop + textAreaHeight / 2, canvas.width - 6, textAreaHeight);
      } else {
...
// Draws a single line, shrinking the font until it fits maxWidth — used
// for the Asset ID under a stacked QR code, where there's no room to wrap
// onto a second line and still be readable.
function drawFittedLine(ctx, text, centerX, centerY, maxWidth, maxFontPx) {
  let fontPx = Math.max(6, Math.min(Math.round(maxFontPx), 22));
  while (fontPx > 6) {
