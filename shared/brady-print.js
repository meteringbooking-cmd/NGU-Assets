...
const DEFAULT_SETTINGS = {
  ...
  substrateHeight: 1, // inches — length along the feed
...
export async function connectBradyPrinter() {
  ...
  if (!printerInstance) {
    ...
    // printer status packets come in (battery, firmware, any error message
    // the printer itself is reporting) and we log it so a failure can be
    // diagnosed from the console instead of guessing.
...
export async function printAssetQrLabel({ qrCode, assetId }) {
  ...
  try {
    ...
    if (ok) {
      ...
      // The printer streams status/error info back over Bluetooth as it
      // goes, and the SDK mirrors the latest of it onto these properties —
      // this is the printer's own explanation, not a guess on our part.
      const status = {
      ...
      const detail = [status.messageTitle, status.message, status.messageRemedy].filter(Boolean).join(" — ");
      toast(
        detail
          ? `Brady printer: ${detail}`
          : `The Brady printer reported the print job failed${status.battery != null ? ` (battery ${status.battery}%${status.onAcPower ? "" : ", not on AC power"})` : ""}. Check the printer and try again.`,
        "error"
      );
    }
    return ok;
