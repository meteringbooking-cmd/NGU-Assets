...
export function buildQrLabel({ qrCode, assetId }) {
  ...
  // the Asset ID for the app's own scan-input boxes.
  if (assetId && window.QRCode) {
    new QRCode(codeHost, { text: `${APP_BASE_URL}/asset-record.html?id=${encodeURIComponent(assetId)}`, width: 200, height: 200 });
  }
  return label;
}
