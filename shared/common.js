...
/**
 * The permanent address this app is deployed at — baked into every
 * printed QR code so scanning one with an ordinary phone camera opens
 * the asset directly, instead of just showing the raw code as text.
 * Update this if the app ever moves to a different domain.
 */
export const APP_BASE_URL = "https://ngu-assets.vercel.app";

/**
 * Builds the URL a printed QR code should encode for a given asset.
 *
 * A URL is a lot more data than the bare code the app used to print, and
 * on a small physical label (this app's labels run as small as 0.5in x
 * 0.5in) a denser QR pattern can print too fine for a phone camera to
 * resolve at all — that's what "the camera won't even pick it up" means.
 * The scheme and host are case-insensitive per the URL spec, so
 * uppercasing just that portion is always safe and lets the QR encoder
 * use its more compact alphanumeric mode there, which measurably shrinks
 * the printed pattern (one QR "version" smaller in testing). The path and
 * query key are deliberately left as-is: Vercel's static hosting is
 * case-sensitive for file paths, and uppercasing them would 404.
 */
export function assetRecordQrUrl(assetId) {
  const full = `${APP_BASE_URL}/asset-record.html?id=${encodeURIComponent(assetId)}`;
  return full.replace(/^https:\/\/[^/]+/, (m) => m.toUpperCase());
}

/**
 * Every printed QR code encodes a full URL (see buildQrLabel below and
 * shared/brady-print.js), not a bare code — so anything a scan-input box
 * or a camera scanner receives needs to be unwrapped back down to the
 * Asset ID before it's looked up. A bare code (typed manually, or from an
 * older printed label) passes through unchanged.
 */
export function extractScannedAssetId(rawValue) {
  ...
  if (/^https?:\/\//i.test(value)) {
    try {
      ...
      // Not a valid URL after all — fall through and treat it as a raw code.
    }
  }
  return value;
}
 ...
 * Asset Categories' CalibrationInterval + CalibrationUnit fields.
 */
...
export function buildQrLabel({ qrCode, assetId }) {
  ...
  // Encodes a link to the asset's record, not the bare code, so scanning
  // the printed label with an ordinary phone camera opens it directly.
  // extractScannedAssetId() on the reading side unwraps this back down to
  // the Asset ID for the app's own scan-input boxes. Error-correction
  // level L (not the library's default M) keeps the printed pattern as
  // sparse as possible — this label is small enough that density matters.
  if (assetId && window.QRCode) {
    new QRCode(codeHost, {
      text: assetRecordQrUrl(assetId),
      width: 200,
      height: 200,
...
const NAV_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: "allocate-asset-mobile", label: "Allocate Asset (Mobile)", href: "allocate-asset-mobile.html", icon: "smartphone" },
